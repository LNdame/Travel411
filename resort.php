	...
	<div class="container">
		<div class="room-bottom">
			...
			<?php 
				...
				$sql_loc = "SELECT DISTINCT LocationName, tbllocations.LocationId  FROM tbllocations INNER JOIN tbltourpackages ON tbllocations.LocationId = tbltourpackages.LocationId WHERE EstTypeId = 1";
...
<div class="rooms">
	...
	<?php 
		...
		if(!isset($_GET['location']) || $_GET['location']==0 ){
			$sql = "SELECT * from tbltourpackages WHERE EstTypeId = 1";
		}else{
			$loc =$_GET['location'];
			$sql = "SELECT * from tbltourpackages WHERE LocationId=$loc  AND EstTypeId = 1";
		}

		//$sql = "SELECT * from tbltourpackages WHERE PackageLocation='$_GET['location']' EstTypeId = 1";
		...
		//$sqlsmallset = "SELECT * from tbltourpackages WHERE EstTypeId = 1 LIMIT " . $this_page_first_result . " , " . $result_per_page;
		...
		if(!isset($_GET['location']) || $_GET['location']==0 ){
			...
			$loc =$_GET['location'];
			$sqlsmallset = "SELECT * from tbltourpackages WHERE LocationId=$loc  AND EstTypeId = 1 LIMIT " . $this_page_first_result . " , " . $result_per_page;
		...
		{	?>
					<div class="rom-btm">
						...
						<div class="col-md-3 room-right wow fadeInRight animated" data-wow-delay=".5s">
							...
							<a href="resort-details.php?pkgid=<?php echo htmlentities($result->PackageId);?>" class="view">Details</a>
						</div>
						<div class="clearfix"></div>
					</div>
